        ...
        <div class="h-full w-2/3 md:w-full overflow-y-auto bg-gray-50 dark:bg-gray-800 ">
            <div class="flex items-center gap-4  px-2 py-5 border-b m-1">
                ...
                <img class="w-10 h-10 p-1 rounded-full ring-2 ring-gray-300 dark:ring-gray-500"
                    :src="userData.profileImage" alt="Bordered avatar">

                <div class="font-medium dark:text-white">
                    <div>{{ userData.name }}</div>
                    <div class="text-xs md:text-sm text-gray-500 dark:text-gray-400">{{ userData.email }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">ID : {{ userData.rollno || userData.teacherId
                        }}</div>
                </div>
            </div>
            ...
            <ul class="space-y-4 font-medium px-3">
                ...
                <li>
                    <router-link :to="{ name: 'Policy' }"
                        ...
                        <span class="flex-1 ms-3 whitespace-nowrap">Privacy & Security Policy</span>
                        <!-- Updated for clarity -->
                    </router-link>
                </li>
                <li>
    ...
    <div v-if="showLogoutModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-sm w-full">
            ...
            <div class="flex justify-end space-x-3">
                <button
                    class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
                    @click="cancelLogout">
                    Cancel
                </button>
...
export default {
    ...
    setup(props) {
        ...
        const confirmLogout = async () => {
            ...
            await db.collection(table).doc(props.userData.id).set({
                status: "out"
            }, {
                merge: true
            });
