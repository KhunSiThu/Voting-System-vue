<template>
    <div v-if="userData" class="text-center">
        ...
        <div class="sm:ml-64 bg-gray-100 text-gray-900 rounded-lg dark:bg-gray-900 dark:text-gray-200">
            ...
            <div
                class="md:hidden flex w-full sticky top-0 left-0 z-40 flex-row justify-between items-center mb-5 bg-white dark:bg-gray-800 p-5 border-b border-gray-100 dark:border-gray-900">
                ...
                <div class="w-3/4 md:w-1/4 flex items-center space-x-3 text-base md:text-xl">
                    <label for="searchInput">Search</label><input v-model="searchValue" id="searchInput"
            ...
            <div v-if="showModal"
                ...
                <div class="relative bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-lg w-full">
                    ...
                    <div class="text-center mb-8">
                        <div class="relative w-32 h-32 mx-auto md:w-60 md:h-60 rounded-full overflow-hidden shadow-lg border-4"
            ...
            <div v-if="showVotingEndModal"
                class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 bg-blur">
                <div
                    class="relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg shadow-2xl rounded-2xl p-10 sm:p-14 w-full sm:w-3/4 lg:w-1/2 text-center border border-gray-300 dark:border-gray-700">
                    <!-- Close Button -->
                    <button @click="showVotingEndModal = false"
                        class="absolute top-4 right-4 bg-gray-300 dark:bg-gray-700 text-gray-700 dark:text-gray-300 p-2 rounded-full hover:bg-red-500 hover:text-white transition-all duration-300">
                        ✕
                    </button>
                    ...
                    <div class="mt-8 flex flex-col sm:flex-row justify-center gap-4">
                        <router-link :to="{ name: 'Results' }">
                            <button
                                class="bg-blue-600 text-white px-8 py-3 rounded-xl font-semibold shadow-md hover:bg-blue-700 transition-all duration-300 transform hover:scale-105 w-full sm:w-auto">
                        ...
                        <router-link :to="{ name: 'Results' }">
                            <button
                                class="bg-purple-600 text-white px-8 py-3 rounded-xl font-semibold shadow-md hover:bg-purple-700 transition-all duration-300 transform hover:scale-105 w-full sm:w-auto">
        ...
        <i class="fas fa-crown cursor-pointer text-3xl" id="kingIcon"></i>
                ...
                <section class="border p-3 dark:border-gray-700 rounded-lg text-center bg-gray-100 dark:bg-gray-900">
                    ...
                    <div v-else id="candidates-show-con"
                        class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3 md:gap-16">
                        <div v-for="candidate in filterCandidates" :key="candidate.rollno"
                            ...
                            :class="(candidate.rollno === userData.voteWholeKing ? 'bg-blue' : (candidate.rollno === userData.voteWholeQueen ? 'bg-pink' : 'bg-white dark:bg-gray-800'))">
                            <div class="relative flex flex-col items-center">
                                <div class="relative w-full h-24 md:h-60 rounded overflow-hidden shadow-lg border"
                                ...
                                <div class="absolute left-2 -bottom-3 md:-bottom-8 items-center text-sm">
                                    <span
                                        class="md:w-16 md:h-16 w-6 h-6  md:text-3xl text-xs font-bold text-white flex items-center justify-center rounded-full mr-3"
                                        :class="candidate.gender === 'male' ? 'bg-blue-500' : 'bg-pink-400'">
                                        {{ candidate.number }}
                                    </span>
                                </div>
                            </div>
                            <h2 class="text-base md:text-2xl font-bold text-blue-800 dark:text-white mt-4 mb-2">{{
                                candidate.name }}
                            </h2>
                            <p class="text-xs md:text-xl text-gray-700 dark:text-gray-300">{{ candidate.major }} Major
                            </p>
                            <button @click="openModal(candidate)"
                                class="mt-3 md:mt-7 w-16 md:w-28 text-xs md:text-lg font-bold hover:text-white border rounded-md md:py-2 py-1 transition"
                                :class="[
