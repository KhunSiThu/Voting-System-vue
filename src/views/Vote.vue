...
<div class="sm:ml-64 bg-gray-100 text-gray-900 rounded-lg dark:bg-gray-900 dark:text-gray-200">
    ...
    <section class="text-center relative bg-cover bg-center pb-10 mx-auto">
        ...
        <div class="bg-white z-10 flex flex-col items-center py-5 px-3 sticky w-full header dark:bg-gray-800">
            <div class="w-60 mb-4 overflow-hidden object-cover border border-gray-300 rounded dark:border-gray-500">
        ...
        <div class="h3 flex justify-center items-center" :class="king_queen === 'KING' ? 'text-blue-400' : 'text-pink-400'">
            ...
            <div v-else id="filter_king" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-12 p-4 border dark:border-gray-700 rounded-lg">
                ...
                <!-- Candidate Card -->
                <div v-for="candidate in filterCandidates" :key="candidate.rollno" class=" p-6 rounded-lg shadow-lg hover:shadow-xl transition duration-300 border dark:border-gray-700" :class="(candidate.rollno === voteMajorKing ? 'bg-blue-100 dark:bg-blue-800' : (candidate.rollno === voteMajorQueen ? 'bg-pink-100 dark:bg-pink-800' : 'bg-white dark:bg-gray-800'))">
                    ...
                    <button @click="openModal(candidate)" class=" m-7 w-28 font-bold hover:text-white border rounded-lg py-2 px-3 transition" :class="[
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 bg-blur">
            <div class="relative bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-lg w-full">
                ...
                <div class="text-center mb-8">
                    <img src="https://img.freepik.com/free-photo/confident-asian-woman-face-portrait-smiling_53876-144815.jpg?ga=GA1.1.518632610.1735269050&semt=ais_hybrid" alt="Candidate Image" class="w-28 h-28 mx-auto rounded-full object-cover shadow-lg border-4" :class="selectedCandidate.gender === 'male' ? 'border-blue-500' : 'border-pink-500'">
...
export default {
    ...
    setup() {
        ...
        let voteMajorKing = ref(localStorage.getItem("voteMajorKing"));
        let voteMajorQueen = ref(localStorage.getItem("voteMajorQueen"));
        ...
        const openModal = async (candidate) => {
            ...
            const collectionName = candidate.gender === 'male' ? "vote-major-king" : "vote-major-queen";
        ...
        const confirmVote = async (selectedCandidate) => {
            ...
            const collectionName = selectedCandidate.gender === "male" ? "vote-major-king" : "vote-major-queen";
            ...
            if (result.success) {
                ...
                if (selectedCandidate.gender === "male") {
                    voteMajorKing.value = selectedCandidate.rollno;
                    localStorage.setItem("voteMajorKing", selectedCandidate.rollno);
                } else {
                    voteMajorQueen.value = selectedCandidate.rollno;
                    localStorage.setItem("voteMajorQueen", selectedCandidate.rollno);
                }
